...
const updateUser = async (req, res) => {
    ...
    await User.findByIdAndUpdate(id, { name, contact_number, role_name }, { new: true })
        .then((data) => {
            res.status(200).send({
                success: true,
...
const getUserAndProjectBySearch = async (req, res) => {
    ...
    const users = await User.find(
        { $and: [{ ...userQuery }, { _id: { $ne: id } }] },
        { password: 0 }
    );
    const usersProject = await Chat.find({
        $and: [{ isGroupChat: true }, { users: id }, { ...projectQuery }],
    }).populate("latestMessage");
    if ((users, usersProject)) {
        res.status(200).send([...users, ...usersProject]);
    } else {
        res.status(403).send({
            message: "Something went wrong",
        });
